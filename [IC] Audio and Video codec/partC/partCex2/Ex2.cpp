...
int main(int argc, char* argv[]){
    ...
    int quantization = atoi(argv[2]); //valor de quantizaçao desejada para o lossy
    ...
    for(int i=0; i<rows; i++){ //cada linha
        for(int j=0; j<columns; j++){ //cada coluna
            matImageIn.at<Vec3b>(i,j)[0] = (matImageIn.at<Vec3b>(i,j)[0] << quantization) >> quantization; //vec3B sao 3 dimensoes (R,G,B), portanto tem tres posicoes: B
            matImageIn.at<Vec3b>(i,j)[1] = (matImageIn.at<Vec3b>(i,j)[1] << quantization) >> quantization; //G
            matImageIn.at<Vec3b>(i,j)[2] = (matImageIn.at<Vec3b>(i,j)[2] << quantization) >> quantization; //R
        }
